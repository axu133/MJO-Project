...
for i in range(3):
    ...
    # Create datasets
    train_val_dataset = OBSDiskDataset(obs_directory, time_dir, lead_times_to_load, indices=np.concatenate([train_indices, val_indices]))
    ...
    train_dataset = torch.utils.data.Subset(train_val_dataset, list(range(0, train_len)))
    val_dataset = torch.utils.data.Subset(train_val_dataset, list(range(train_len, N)))
